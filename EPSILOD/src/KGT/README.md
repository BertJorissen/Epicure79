EPSILOD Kernel Generator Tool
====
This folder contains the tool to automatically generate stencil kernels to use with EPSILOD. To generate a kernel, use `epsilod_kgt.sh`, providing the stencil description file as argument.
...
**NOTE:** EPSILOD has a generic kernel that can compute any kind of stencil representable by the specified three elements, without the need to define a specific kernel code. That kernel is used by the skeleton function when a `NULL` value is provided as the parameter for the pointer to the kernel to compute. However, any kernel code generated with this tool will achieve higher performances than those reachable by the generic kernel.
...
Currently, the tool presents the following restrictions:
 ...
 - Only GENERIC-compatible kernels (i.e. kernels that work with any Controllers backend) can be generated.
