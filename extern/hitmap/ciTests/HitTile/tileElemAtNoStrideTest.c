/**
 * Test for HitTile function hit_tileElemAtNoStride
 * 
 * Cases checked:
 *      * 1D HitTile
 *      * 2D HitTile
...
int main() {
    ...
    HitSig sig = hit_sig(1, 20, 1);     // card = 10
    int ok = 1, i, j;
    ...
    printf("Check hit_tileElemAtNoStride: ");
    ...
    //
    printf("\t1D: ");
    for(i=0; i<10; i++) hit_tileElemAtNoStride(tile1D, 1, i) = i;
    ...
    ok = 1;
    for(i=0; i<10; i++)
        for(j=0; j<10; j++) hit_tileElemAtNoStride(tile2D, 2, i, j) = j+i*10;
    ...
    ok = 1;
    for(i=0; i<10; i++)
        for(j=0; j<10; j++) hit_tileElemAtNoStride(tile3D, 3, i, j, 2) = j+i*10;
    ...
    ok = 1;
    for(i=0; i<10; i++)
        for(j=0; j<10; j++) hit_tileElemAtNoStride(tile4D, 4, 3, i, j, 2) = j+i*10;
