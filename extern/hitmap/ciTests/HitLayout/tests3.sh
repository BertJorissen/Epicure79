...
execute() {
   ...
   if [ "$sourcefile" == "" ]; then
      ...
      if [ ! -f "$sourcefile" ]; then
         #echo -e "$YELLOW""$file $RESET"": no such file $cfile"
   ...
   for numprocs in $PROCS; do
      ...
      errors=''
      let ran=$ran+1
      # echo -e "$YELLOW""$testFile"  $numprocs "$RESET_COLOR" >>testsoutput.txt
      if [ $verbose == 0 ]; then
      ...
      srun -n $numprocs -w manticore ./$exefile > ${tmpfile} # 2>>testserrors.txt
