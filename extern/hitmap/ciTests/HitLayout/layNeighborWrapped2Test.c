 ...
 * hit_layNeighbor3 and hit_layNeighborN (wrapped)
 * 
 ...
 *          Expected result: if the neighbor index is out of bounds, the neighbor
 *              search starts again from index 0
...
int main(int argc, char *argv[]) {
    ...
    if (hit_layImActive(lay)){
        ...
        if (n >= hit_layDimNumActives(lay, 1)) n = 0;
    }
    else{
        m = myRanks.rank[0];
        n = myRanks.rank[1];
    ...
    if (hit_layImActive(lay)){
        ...
        if (m >= hit_layDimNumActives(lay, 0)) m = 0;
        if (n >= hit_layDimNumActives(lay, 1)) n = 0;
        if (o >= hit_layDimNumActives(lay, 2)) o = 0;
    }
    else{
        m = myRanks.rank[0];
        n = myRanks.rank[1];
