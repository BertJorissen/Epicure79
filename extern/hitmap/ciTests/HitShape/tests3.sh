...
execute() {
   ...
   if [ $verbose == 0 ]; then
      echo "$file  $numprocs " >> testsoutput.txt
   else
      echo -e "$YELLOW""$file"  $numprocs "$RESET_COLOR"
   fi

   srun -n $numprocs -w manticore ./$file > ${tmpfile} # 2>>testserrors.txt
...
## ALL FILES
   ...
   for testFile in $t; do
      noextension=$(echo $testFile | cut -d. -f1)
      cfile="$noextension""Test.c"

      if [ ! -f "$cfile" ]; then
         echo -e "$testFile: no such file $cfile"
         echo
         #exit 1
