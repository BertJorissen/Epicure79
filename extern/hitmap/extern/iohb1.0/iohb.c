...
/**
 * Fix some memory leaks 18-4-2011, by Javier Fresno.
 * More memory leaks 4-4-2012, by Javier Fresno.
 */
...
/*************************************************/
    ...
    *perline = atoi(tmp);
    ...
    if ( strchr(fmt,'.') ) {
	  ...
	  // Javier
      // Previous: *prec = atoi( substr( fmt, strchr(fmt,'.') - fmt + 1, strchr(fmt,')') - strchr(fmt,'.')-1) );
	...
	// Javier
	// Previous: return *width = atoi(tmp);
	*width = atoi(tmp);
	if(tmp != NULL) free(tmp);
	return *width;
