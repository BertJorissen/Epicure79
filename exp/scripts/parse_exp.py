...
# TODO: care for non res fields that contain str res or sum and same for clock
...
for machine in os.scandir(results_raw_path):
	for bench in os.scandir(os.path.join(machine.path, "raw")):
		for arch in os.scandir(bench.path):
			...
			data_dict=dict()
			for version in os.scandir(arch.path):
					...
					for l in [l.split(", ") for l in f_in.readlines()]:
						...
						if key in data_dict:
							if data_dict[key]["res"] != tuple(l[key_lenght:key_lenght+res_lenght]):
								print(f"error results differ on {key}")
			...
			x_axis_parameter = "baseline" if "sobel" in bench.name else "size"
			...
			for key, value in grouped_dict.items():
				for x_axis_value in value.keys():
					if ref_res_pair[1][x_axis_value]["res"] != value[x_axis_value]["res"]:
						print(f"error results differ on {ref_res_pair[0]} and {key} on size {x_axis_value}")
			...
			for key, value in grouped_dict.items():
				out_file=os.path.join(machine.path, "stats", bench.name, arch.name, "_".join([s.replace(" ", "-") for s in key]) + ".csv")
